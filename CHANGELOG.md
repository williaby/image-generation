...
- `.pre-commit-config.yaml` `astral-sh/ruff-pre-commit` rev bumped from
  ...
  ruff formatter disagreed with the project's modern ruff on a handful of
  test files, so `pre-commit run --all-files` locally would reformat files
  that current ruff considered already-formatted. With the pins aligned,
  pre-commit and CI agree on canonical formatting. SHA-pin format preserved
  per PR #18 PC-* compliance work. Discovered while triaging local
  pre-commit drift during PR #26 review.
